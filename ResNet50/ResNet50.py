...
DATA_FOLDER = r'G:\OCIF\Datasets'
TRAIN_PATH = os.path.join(DATA_FOLDER, "training") # Path for training data
VALID_PATH = os.path.join(DATA_FOLDER, "validation") # Path for validation data
NUMBER_OF_CLASSES = len(os.listdir(TRAIN_PATH)) # Number of classes of the dataset
RESULTS_PATH = 'results.csv' # The path to the results file

# Creating generators from training and validation data
batch_size=1 # the mini-batch size to use for the dataset
...
def get_model(num_layers,con_cat,con_cat1,con_cat2,con_cat3, num_neurons, dropout, activation, weight_initializer):
    base_model = ResNet50(weights="imagenet") # make an instance of an existing architecture
    ...
    model = models.Model(inputs=[base_model.inputs[0],Y],outputs=X) # making a keras Model instance for training
...
try:
    ...
    log_df = pd.DataFrame(columns=['index', 'activation','concat','concat1','concat2','concat3', 'weight_initializer', 'dropout', 'num_neurons', 'num_layers', 'train_loss', 'train_acc', 'val_loss', 'val_acc'])
...
for combo in p_space:
    print(combo)
    activation, weight_initializer, num_layers,concat,concat1,concat2,concat3 = combo
    ...
    for i in range(num_layers): # add dropout ranges based on number of layers
        bounds.append({'name': 'dropout' + str(i + 1), 'type': 'discrete', 'domain': numpy.arange(0, 0.6, 0.3)})
    for i in range(num_layers): # add number of neurons ranges based on number of layers
        bounds.append({'name': 'num_neurons' + str(i + 1), 'type': 'discrete', 'domain': [2 ** j for j in range(5, 10)]})
    ...
    def model_fit(x):
        ...
        history = model.fit_generator(train_generator,
            validation_data=valid_generator, epochs=50, callbacks=[lr_reducer],
            steps_per_epoch=5700,
        ...
        log_tuple = (activation,concat,concat1,concat2,concat3, weight_initializer, dropouts, neurons, num_layers, history.history['loss'][best_acc_index], history.history['acc'][best_acc_index], history.history['val_loss'][best_acc_index], history.history['val_acc'][best_acc_index])
